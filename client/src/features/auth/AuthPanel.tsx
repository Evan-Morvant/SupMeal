...
import { oauthAuthorizeUrl } from '../../auth/auth.api';
import { Logo } from '../../ui/Logo';
import styles from './AuthLayout.module.css';

/*
 * Cadre commun à la connexion et à l'inscription, qui ne diffèrent que par
 * leur formulaire.
 */

/** Marque GitHub, telle que publiée pour les boutons de connexion. */
...
/** Marque Google, dans ses quatre couleurs officielles. */
...
interface AuthPanelProps {
  title: string;
  subtitle: string;
  /** Verbe des boutons de fournisseur : « Se connecter avec… ». */
  providerVerb: string;
  children: ReactNode;
  /** Renvoi vers l'autre écran d'entrée. */
  swap: ReactNode;
}
    ...
    <div className={styles.wrap}>
      <div className={styles.panel}>
        ...
        <div className={styles.providers}>
          <a className={styles.provider} href={oauthAuthorizeUrl('google')}>
            <GoogleMark />
            {providerVerb} avec Google
          </a>
          <a className={styles.provider} href={oauthAuthorizeUrl('github')}>
            <GithubMark />
            {providerVerb} avec GitHub
          </a>
        </div>

        <p className={styles.swap}>{swap}</p>
      </div>
    </div>
  );
}
